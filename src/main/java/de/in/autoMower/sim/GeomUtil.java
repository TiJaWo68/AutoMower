...
public final class GeomUtil {
	 ...
	 * Note : angle should be computed with "Math.atan2(ptB.getY() - ptA.getY(), ptB.getX() - ptA.getX())" in an ortho-normal basis system where
	 * positive angle is defined in a CounterClockWise orientation.
	 *
	 ...
	 * @param k   represents ratio between AB and AC, ptC being the returned point along AB line segment.<br>
	...
	public static Point2D getIntersectPoint(Line2D line, Rectangle2D rect) {
		...
		if (p == null) {
			p = lineIntersection(line, new Line2D.Double(rect.getMinX(), rect.getMaxY(), rect.getMaxX(), rect.getMaxY()));
			if (p == null) {
				p = lineIntersection(line, new Line2D.Double(rect.getMinX(), rect.getMinY(), rect.getMinX(), rect.getMaxY()));
				if (p == null)
					p = lineIntersection(line, new Line2D.Double(rect.getMaxX(), rect.getMinY(), rect.getMaxX(), rect.getMaxY()));
			}
		}
		return p;
	}
	...
	public static boolean lineParallel(Point2D ptA, Point2D ptB, Point2D ptC, Point2D ptD) {
		...
		return MathUtil.isEqualToZero((ptB.getX() - ptA.getX()) * (ptD.getY() - ptC.getY()) - (ptB.getY() - ptA.getY()) * (ptD.getX() - ptC.getX()));
	}

	public static boolean lineColinear(Point2D ptA, Point2D ptB, Point2D ptC, Point2D ptD) {
		if (lineParallel(ptA, ptB, ptC, ptD))
			return MathUtil.isEqualToZero((ptA.getY() - ptC.getY()) * (ptD.getX() - ptC.getX()) - (ptA.getX() - ptC.getX()) * (ptD.getY() - ptC.getY()));
		return false;
	}
	...
	public static Point2D getCircleCenter(List<Point2D> ptList) {
		...
		case 2 -> new Point2D.Double((ptList.get(0).getX() + ptList.get(1).getX()) / 2.0, (ptList.get(0).getY() + ptList.get(1).getY()) / 2.0);
		default -> null;
		};
	}
	 ...
	 * This function handle cases of mirror image flip about some axis. This changes right-handed coordinate system into a left-handed system.
	 * Hence, returned angle has an opposite value.
	 *
